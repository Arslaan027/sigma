...
const Step5 = ({ nextStep, prevStep, setOffice }) => {
  const [selectedOffice, setSelectedOffice] = useState("Physical office");
    ...
    <div className="w-full max-w-lg mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="flex items-center mb-4 text-gray-700">
        ...
        <h2 className="ml-4 text-xl font-bold">
          Do you need a physical office, or will you use a virtual office?
        </h2>
      </div>
      <div className="space-y-4">
        {["Physical office", "Virtual office", "No office required"].map(
          (option) => (
            <button
              key={option}
              onClick={() => setSelectedOffice(option)}
